...
-- Check if target is within Box FOV
...
-- Sync the box with the predicted target position
...
-- Check if the current tool is a knife or katana (for weapon filtering)
...
-- Check if mouse is on the target player
...
-- TriggerBot action - only fires when the Silent Aimbot is targeting
local function TriggerBotAction()
    if TargetPlayer and TargetPlayer.Character then
        ...
        if humanoid and humanoid.Health > 0 and not isPlayerKnocked(TargetPlayer) then
            -- Safety checks (e.g., ignoring knife)
            ...
            if isMouseOnTarget(TargetPlayer) then
                ...
                if getgenv().TriggerBot.Settings.Preds.EnableLegitMode then
                    predictedPosition = getPredictedPosition(TargetPlayer.Character)
                else
                    predictedPosition = TargetPlayer.Character.PrimaryPart.Position
                end

                if predictedPosition and isWithinBoxFOV(predictedPosition) then
                    -- Sync the box with the target’s predicted position
                    ...
                    if os.clock() - lastClickTime >= getgenv().TriggerBot.Settings.Cooldown then
                        ...
                        -- Fire the weapon if it’s not a knife/katana
...
-- Toggle TriggerBot on button press
UserInputService.InputBegan:Connect(function(input, gameProcessed)
...
-- Disable TriggerBot on button release
...
-- Continuous check for TriggerBot activation
RunService.RenderStepped:Connect(function()
    if isToggled and getgenv().Silent.Startup.Enabled then
        TriggerBotAction()  -- TriggerBot only operates when Silent Aimbot is enabled
    end
end)
